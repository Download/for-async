...
/***/ function(module, exports, __webpack_require__) {
	...
	var _forAsync = __webpack_require__(43);
	
	var _forAsync2 = _interopRequireDefault(_forAsync);
	
	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
	
	describe('forAsync', function () {
		it('can be used to iterate over an array', function (done) {
			var arr = ['one', 'two', 'three'];
			var expectedResult = arr.join('');
			var result = '';
			(0, _forAsync2.default)(arr, function (item, idx) {
				result += item;
			}).then(function () {
				(0, _chai.expect)(result).to.equal(expectedResult);
				done();
			});
		});
	
		it('can execute async work on each item', function (done) {
			var arr = ['one', 'two', 'three'];
			var expectedResult = arr.join('');
			var result = '';
			(0, _forAsync2.default)(arr, function (item, idx) {
		...
		it('passes `item` and `idx` to the callback on each iteration', function (done) {
			var arr = ['one', 'two', 'three'];
			var expectedResult = arr.join('');
			var expectedIndexes = "012";
			var result = '';
			var resultIndexes = '';
			(0, _forAsync2.default)(arr, function (item, idx) {
				return new Promise(function (resolve) {
				...
				(0, _chai.expect)(result).to.equal(expectedResult);
				(0, _chai.expect)(resultIndexes).to.equal(expectedIndexes);
				done();
			});
		});
	
		it('can be aborted by throwing from sync code', function (done) {
			var arr = ['one', 'two', 'three'];
			(0, _forAsync2.default)(arr, function (item, idx) {
				...
				(0, _chai.expect)(error.message).to.equal('aborted');
				done();
			});
		});
	
		it('can be aborted by rejecting the promise from async code', function (done) {
			var arr = ['one', 'two', 'three'];
			(0, _forAsync2.default)(arr, function (item, idx) {
...
/***/ }
/******/ ]);
//# sourceMappingURL=for-async.spec.js.map